...
function TownsLeaderboard({reloadTownPerks, updateBalance, updateXP}) {
    ...
    const townLeaderboardSlot = (town, position) => {
        ...
        switch (position) {
            case 1:
                topClass = 'townFirst'; break;
            case 2:
                topClass = 'townSecond'; break;
            case 3:
                topClass = 'townThird'; break;
        }
        let townLevel = calcTownLevel(town.townXP)[0]

        return (<div key={position} className='townLeaderboardSlot'>
            ...
            >
                <div className='lbCardLeft'>
                    <img className='lbTownLogo' src={`${process.env.PUBLIC_URL}/assets/images/townIcons/${TOWNSINFO.townIcons[town.townLogoNum]}.png`} />
                    <p className='lbTownName'>{town.townName}</p>
                    <p className='lbTownLevel'>town lvl {townLevel}</p>
                </div>
                <div className='lbCardRight'>
                    <p className={town.status === 'OPEN' ? 'lbTownStatus' : ''}>{town.status}</p>
                    <p className='lbMemberCount'>{town.memberCount}/25</p></div>
            </div>
        </div>)
    }
        ...
        <div className='townsLeaderboardContainer'>
            {viewing &&
                <div className='searchViewInterface'>
                    <TownInterface reloadTownPerks={reloadTownPerks} updateBalance={updateBalance} updateXP={updateXP} townName={viewing}
                        backArrow={() => { setViewing("") }}
                    />
                </div>
            }
