...
function GoalCard({
  ...
  let percentCompletion =
    numHave >= numNeeded ? 100 : Math.floor((numHave / numNeeded) * 100);
  ...
  return (
    <div className="goalCardContainer">
      ...
      <img
        className={`goalCardIcon ${setByLeader ? "customGoal" : ""} ${
          myRoleID > 2 && setByLeader ? "clickable" : ""
        }`}
        src={`${process.env.PUBLIC_URL}/assets/images/${good}.png`}
        onClick={() => {
          if (setByLeader && myRoleID > 2) {
            setGoalSelect(true);
          }
        }}
        onMouseEnter={() => {
          toolTip.current = setTimeout(
            () => {
              setTool(true);
            },
            myRoleID > 2 ? 800 : 500,
      ...
      />
      {tool && setByLeader && (
        <p className="goalToolTip">Goal set by leader or co-leader</p>
      )}
