...
function CompShop({
  ...
  const isUnlocked = (itemName) => {
    let permit = "";
    if (
      CONSTANTS.Permits.deluxeCrops.includes(itemName) &&
      !upgrades.hasDeluxePermit
    ) {
      permit = "Deluxe";
    } else if (
      CONSTANTS.Permits.exoticAnimals.includes(itemName) &&
      !upgrades.hasExoticPermit
    ) {
      permit = "Exotic";
    }
  ...
  const getAnimalItems = () => {
    ...
    allAnimals = allAnimals.sort(
      (a, b) =>
        CONSTANTS.shopOrder.indexOf(a[0]) - CONSTANTS.shopOrder.indexOf(b[0]),
    );
    ...
    for (let i = 0; i < allAnimals.length; ++i) {
      ...
      let hasSpace =
        animals[`${allAnimals[i][1][0]}Count`] <
        animals[`${allAnimals[i][1][0]}Capacity`];
      if (unlockInfo[0]) {
        if (hasSpace) {
          firstItems.push(
            <CompItem
              ...
              getBal={getBal}
              itemName={name}
              cost={allAnimals[i][1][1]}
              unlocked={true}
              info={`${allAnimals[i][1][0].toUpperCase()} ${unlockInfo[1]}`}
              hasSpace={hasSpace}
            />,
          );
        } else {
      ...
      if (allUpgrades[i].unlocked) {
        firstItems.push(
          <CompItem
            ...
            updateBalance={updateBalance}
            getBal={getBal}
            itemName={allUpgrades[i].name}
            cost={allUpgrades[i].cost}
            unlocked={true}
            info={info}
            tier={allUpgrades[i].tier}
          />,
        );
      } else {
        lastItems.push(
