...
function ChatBox({ setMsgNotification, chatType, targetName, closeMethod, chatMessages }) {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {  // Check if 'Enter' was pressed and ensure 'Shift' wasn't held down
    ...
    const sendMessage = async () => {
        if (waitForServerResponse) {
            if (chatType === "TOWN") {
                let msgResult = await waitForServerResponse('createTownMessage', { messageContent: newMessage });
                ...
                <span
                    onClick={() =>
                        navigate(`/profile/${userWhoSent}`, {
                            state: {
                                from:
                                    location.pathname.substring(1, location.pathname.length).includes('profile') ?
                                        'plants'
                                        :
                                        location.pathname.substring(1, location.pathname.length)
                            }
                        })}
                    className='fromUser'>{userWhoSent}:</span>
