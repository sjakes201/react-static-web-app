...
function ScrollingText() {
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(lbPositions).length !== 0
    ) {
      let newSplashes = [];
      ...
      for (let i = 0; i < allSpots.length; ++i) {
        ...
        if (category === "Balance") {
          if (position === 1) {
            newSplashes.push(
              `👑You are the richest player in the game. Finally on top. 👑`,
            );
          } else if (position === 2) {
            newSplashes.push(
            ...
            newSplashes.push(
              `Congrats on being the ${position}${ordinalSuffix(
                position,
              )} richest player in the game. If only this game had a stock market.`,
            );
          } else {
            newSplashes.push(
          ...
          if (position === 1) {
            newSplashes.push(
              `Number one ${CONSTANTS.InventoryDescriptions[category][0]} farmer in the world! Victory is yours, as it always should have been.`,
            );
          } else if (position === 2) {
            newSplashes.push(
              `2nd best ${CONSTANTS.InventoryDescriptions[category][0]} farmer in the world! That must be frusturating.`,
            );
          } else if (position === 3) {
            newSplashes.push(
              `3rd best ${CONSTANTS.InventoryDescriptions[category][0]} farmer in the world! Finally getting the recognition you deserve.`,
            );
          } else if (position >= 4 && position <= 10) {
            newSplashes.push(
              `${position}${ordinalSuffix(position)} best ${
                CONSTANTS.InventoryDescriptions[category][0]
              } farmer! You're so close.`,
            );
          } else {
            newSplashes.push(
              `${position}${ordinalSuffix(position)} best ${
      ...
      for (let i = 0; i < allTotals.length; ++i) {
        ...
        if (num === 0) {
          newSplashes.push(
            `0 ${CONSTANTS.InventoryDescriptionsPlural[category][0]} produced! Someone clearly doesn't like ${CONSTANTS.InventoryDescriptionsPlural[category][0]}.`,
          );
        } else if (num >= 1 && num <= 10) {
          newSplashes.push(
            `Only ${num?.toLocaleString()} ${
              CONSTANTS.InventoryDescriptions[category][0]
            } ever produced!`,
          );
        } else {
          newSplashes.push(
            `${num?.toLocaleString()} ${
              CONSTANTS.InventoryDescriptionsPlural[category][0]
            } produced!`,
          );
        }
      }
